...
import { resolveDmxColorTokens } from "./color.js";
import {
  distanceFromCenter,
  manhattanDistance,
  normalizedRadius,
...
import { clamp01Dmx, getMatrix5Layout, resolveDmxBoxOuterDim } from "./layout.js";
import { getPatternIndexes, indexToCoord, MATRIX_SIZE } from "./patterns.js";
import type {
...
export function DotMatrixBase({
  ...
  // CSS-var-rich style objects don't fit plain CSSProperties (custom props
  // are typed as unknown by React). Widen with a precise intersection
  // instead of `as unknown as` double assertion.
  const dmxVarStyle: CSSProperties &
    Record<`--dmx-${string}`, string | number> = {
    width: matrixSpan,
    height: matrixSpan,
