...
/**
 * Long-memory variant of `recallTopK` (S3) — the SAME active/model/dim/expiry
 * vector recall, but the SELECT/DTO ALSO returns `source` (provenance tier) and
 * `maturity_state` so `long_memory_search` can rank inferred/hypothesis entries
 * LOWER without excluding them (genesis §951). Additive — `recallTopK` /
 * `knowledge_recall` are untouched.
 *
 * Fetches `k * 2` raw candidates (same headroom as `recallTopK`); the caller
 * reranks + de-weights before capping inline.
 */
