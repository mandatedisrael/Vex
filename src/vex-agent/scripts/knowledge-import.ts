 ...
 *
 * Usage:
 *   pnpm exec tsx src/vex-agent/scripts/knowledge-import.ts [--in FILE]
 */
...
import { MaintenanceActiveError } from "@vex-agent/db/repos/maintenance-lease.js";
  ...
  for await (const rawLine of source) {
    ...
    if (!manifestSeen) {
      ...
      if (
        ...
        parsed === null ||
        (parsed as Record<string, unknown>).__type !== "vex_knowledge_export"
      ) {
        throw new Error(
          `line 1: expected manifest with __type="vex_knowledge_export", got ${JSON.stringify(parsed).slice(0, 200)}`,
        );
      }
